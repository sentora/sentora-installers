...
if [ -f /etc/centos-release ]; then
    ...
    OS="Fedora"
    VER=$(rpm -q --queryformat '%{VERSION}\n' fedora-release) # return fedora version numbers
...
#--- Adapt repositories and packages sources for CentOS
...
#--- Adapt repositories and packages sources for Fedora
# for Fedora metalink = mirrorlist
if [[ "$OS" = "Fedora" ]]; then
# disable priority of mirror
sed -i 's|failovermethod=priority|#failovermethod=priority|' "/etc/yum.repos.d/fedora.repo"
sed -i 's|failovermethod=priority|#failovermethod=priority|' "/etc/yum.repos.d/fedora-updates.repo"
...
sed -i 's|#baseurl=|baseurl=|' "/etc/yum.repos.d/fedora-updates.repo"
    ...
    # security disable fedora testing repository
    disablerepo "fedora-updates-testing"

    # We need to disable SELinux...
    # Solve Error /etc/selinux/config not found on CentOS and Fedora (openvz)
    $PACKAGE_INSTALLER -y selinux-policy-targeted
    sed -i 's/SELINUX=enforcing/SELINUX=disabled/g' /etc/selinux/config
    setenforce 0

    # Stop conflicting services and iptables to ensure all services will work
    if  [["$OS" = "Fedora" || "$VER" = "7" ]]; then
    # install sendmail services required to stop sendmail
    $PACKAGE_INSTALLER -y sendmail-sysvinit
    # to avoid the problem in the system
    # the redhat developer recommends
    # based on following version and +
    # Red Hat entreprise 7
    # CentOS 7
    # Fedora 17 and +
    # the command chkconfig and service on it's system
    # are just simple redirect to systemctl
    # Therefore it is strongly recommended to use directly systemctl
    # Following the script will be updated accordingly
    ...
    # disable firewall
    if  [["$OS" = "Fedora" ]]; then
    # install iptables services required to stop iptables
    $PACKAGE_INSTALLER -y iptables-services
